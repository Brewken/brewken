/*======================================================================================================================
 * trees/TreeView.cpp is part of Brewken, and is copyright the following authors 2009-2024:
...
///
///   this->setExpanded(findElement(nullptr), true);
...
///   auto newSelected = this->deleteItems(selected);
///
///   if (newSelected && newSelected->isValid()) {
///      TreeNode * node = this->model().item(*newSelected);
...
///   return this->m_model->classifier(this->m_filter->mapToSource(index));
///}
...
///// ** This should all go into init() and doInit() **
...
///// with users.
///QModelIndex TreeView::deleteSelected(QModelIndexList selected) {
///   //.:TODO:. Pull out some of the common code from this and copySelected()
///
///   int firstRowToDelete = selected.row();
///   int firstColToDelete = selected.column();
///
...
///      }
///
///      // If we have already said "Yes To All", just append and go
///      if (confirmDelete == QMessageBox::YesToAll) {
...
///
///      // Otherwise prompt
///      TreeNode * nodeToDelete = this->m_model->treeNode(trans);
///      confirmDelete = verifyDelete(confirmDelete, nodeToDelete->localisedClassName(), nodeToDelete->name());
...
///   //
///   //
///   QModelIndex newSelected = this->m_model->index(firstRowToDelete, firstColToDelete);
///   if (!newSelected.isValid()) {
///      newSelected = this->m_model->index(0, 0);
///   }
///   return newSelected;
///}
