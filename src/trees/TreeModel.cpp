/*======================================================================================================================
 * trees/TreeModel.cpp is part of Brewken, and is copyright the following authors 2009-2024:
...
///
///   std::optional<QString> getFolderPath(NamedEntity const * ne) {
///      if (!ne) {
///         return std::nullopt;
///      }
///
///      QMetaObject const * neMetaObject = ne->metaObject();
///      int propertyIndex = neMetaObject->indexOfProperty(*PropertyNames::FolderBase::folderPath);
///      if (propertyIndex >= 0) {
///         QMetaProperty neMetaProperty = neMetaObject->property(propertyIndex);
///         Q_ASSERT(neMetaProperty.isReadable());
///         QVariant val = ne->property(*PropertyNames::FolderBase::folderPath);
///         return val.toString();
///      }
///      return std::nullopt;
///   }
///
///   void setFolderPath(NamedEntity * ne, QString const & val) {
///      if (!ne) {
///         return;
///      }
///      QMetaObject const * neMetaObject = ne->metaObject();
...
///         Q_ASSERT(neMetaProperty.isReadable());
///
///         bool succeeded = ne->setProperty(*PropertyNames::FolderBase::folderPath, val);
///         if (!succeeded) {
///            qCritical() <<
///               Q_FUNC_INFO << "Error trying to set" << PropertyNames::FolderBase::folderPath << "on" <<
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-recipe";
///      m_maxColumns = TreeItemNode<Recipe>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-recipe";
///      m_maxColumns = TreeItemNode<Equipment>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-ingredient";
///      m_maxColumns = TreeItemNode<Fermentable>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-ingredient";
///      m_maxColumns = TreeItemNode<Hop>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-ingredient";
///      m_maxColumns = TreeItemNode<Misc>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-recipe";
///      m_maxColumns = TreeItemNode<Style>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-ingredient";
///      m_maxColumns = TreeItemNode<Yeast>::NumberOfColumns;
...
///      m_mimeType = DEF_CONFIG_MIME_PREFIX "-ingredient";
///      m_maxColumns = TreeItemNode<Water>::NumberOfColumns;
///   } else {
...
///TreeNode * TreeModel::item(QModelIndex const & index) const {
///   if (index.isValid()) {
...
///      maxColumns = static_cast<int>(TreeFolderNode<Hop>::NumberOfColumns;
///   } else {
...
///      auto folderPath = FolderUtils::getFolderPath(elem);
///      if (folderPath && !folderPath->isEmpty()) {
///         ndxLocal = findFolder(*folderPath, rootItem->child(0), true);
...
///   auto const notes = recurse ? RecipeHelper::brewNotesForRecipeAndAncestors(*rec) : rec->brewNotes();
...
///   // TODO: At some point we should refactor this code so that we have separate handling for objects that have
///   //       folders from ones that don't.
///   auto folder = FolderUtils::getFolderPath(test);
...
///            auto folder = FolderUtils::getFolderPath(item);
///            if (folder) {
///               FolderUtils::setFolderPath(item, targetPath);
///            }
///         }
///      }
///   }
...
///   } else if (data->hasFormat(DEF_CONFIG_MIME_PREFIX "-folder")) {
///      encodedData = data->data(DEF_CONFIG_MIME_PREFIX "-folder");
///   } else {
...
///         return false;
///      }
///      auto folder = FolderUtils::getFolderPath(something);
///      if (!folder) {
...
///         FolderUtils::setFolderPath(item, target);
...
///   types << m_mimeType << DEF_CONFIG_MIME_PREFIX "-folder";
///
///   return types;
///}
