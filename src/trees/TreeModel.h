/*======================================================================================================================
 * trees/TreeModel.h is part of Brewken, and is copyright the following authors 2009-2025:
 ...
 *        possible to add \c Hop items to a tree of \c Misk items).
 *
 *        A \c TreeModel object "owns" the logical structure of the tree, and provides the standard Qt interface for
 *        interacting with a data model.  The items in the model are subclasses of \c TreeNodeBase (as explained in more
 *        detail in the comments in that class).  Although the parent-child links between these items effectively define
 *        the structure of the tree, there is a more generic location mechanism whereby any item in the model can be
 *        located by a \c QModelIndex object (which only remains valid until the model is altered - like an array index
 *        etc).  See more on this below.
 *
 ...
 *                RecipeTreeModel
 *
 *        Additionally, \c TreeView can use \c TreeFilterProxyModel (which inherits from \c QSortFilterProxyModel) to
 *        sort and filter what is shown in the tree.
 *
 ...
 *           - \c internalPtr is an untyped (ie \c void) raw pointer to the underlying tree node.  We always cast it
 *             back to \c TreeNode, and then call \c classifier() to find out the actual node type.
...
///   QModelIndexList allChildren(QModelIndex parent);
   ...
   virtual bool lessThan(QModelIndex const & left, QModelIndex const & right) const = 0;
