/*======================================================================================================================
 * print/Page.h is part of Brewken, and is copyright the following authors 2021:
...
namespace Print
{
   ...
   public:
      ...
      void placeRelationalTo(T *targetObj, S *sourceObj, RelationalPlacingFlags place, int xOffset = 0 /* pixels */, int yOffset = 0 /* pixels */)
      {
         ...
         y = (place & Print::ABOVE) ? y - target->getBoundingBox().height() : y;
         y = (place & Print::BELOW) ? y + other->getBoundingBox().height() : y;
         x = (place & Print::LEFTOF) ? x - target->getBoundingBox().width() : x;
         x = (place & Print::RIGHTOF) ? x + other->getBoundingBox().width() : x;
      ...
      void placeOnPage(T *targetObj, FixedPlacingFlags place, int xOffset = 0, int yOffset = 0)
      {
         ...
         x = (place & Print::RIGHT) ? pagePaintRect.width() - tO->getBoundingBox().width() : x;
         x = (place & Print::LEFT) ? 0 : x;
         y = (place & Print::TOP) ? 0 : y;
         y = (place & Print::BOTTOM) ? pagePaintRect.height() - tO->getBoundingBox().height() : y;
         y = (place & Print::VCENTER) ? ((pagePaintRect.height() - tO->getBoundingBox().height()) / 2) : y;
         x = (place & Print::HCENTER) ? ((pagePaintRect.width() - tO->getBoundingBox().width()) / 2) : x;
