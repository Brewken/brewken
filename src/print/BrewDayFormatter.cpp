/*======================================================================================================================
 * print/BrewDayFormatter.cpp is part of Brewken, and is copyright the following authors 2021:
...
QString BrewDayFormatter::buildTitleHTML(bool includeImage)
{
   ...
   body += QString("<tr><td class=\"left\">%1</td><td class=\"value\">%2</td><td class=\"right\">%3</td><td class=\"value\">%4</td></tr>")
            .arg(tr("Boil Time"))
            .arg((recObs->equipment()) ? Brewken::displayAmount(recObs->equipment()->boilTime_min(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::boilTime_min, &Units::minutes) : "unknown" )
            .arg(tr("Efficiency"))
   ...
   body += QString("<tr><td class=\"left\">%1</td><td class=\"value\">%2%</td><td class=\"right\">%3</td><td class=\"value\">%4</tr>")
            ...
            .arg( Brewken::displayAmount(Brewken::getVolumeUnitSystem() == SI ? recObs->calories33cl() : recObs->calories12oz(),nullptr,0) );
...
QList<QStringList> BrewDayFormatter::buildTitleList()
{
   ...
   row.append((recObs->equipment()) ? Brewken::displayAmount(recObs->equipment()->boilTime_min(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::boilTime_min, &Units::minutes) : "unknown");
   row.append(tr("Efficiency"));
   row.append(Brewken::displayAmount(recObs->efficiency_pct(),nullptr,0));
   ret.append(row);
   row.clear();

   // third row: pre-Boil Volume and Preboil Gravity
   row.append(tr("Boil Volume"));
   row.append(Brewken::displayAmount(recObs->boilVolume_l(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::boilVolume_l, &Units::liters,2));
   row.append(tr("Preboil Gravity"));
   row.append(Brewken::displayAmount(recObs->boilGrav(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::og, &Units::sp_grav, 3));
   ret.append(row);
   row.clear();
   ret.append(row);
   row.clear();
   ...
   row.append( Brewken::displayAmount(recObs->finalVolume_l(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::finalVolume_l, &Units::liters,2) );
   row.append( tr("Starting Gravity") );
   row.append( Brewken::displayAmount( recObs->og(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::og, &Units::sp_grav, 3 ) );
   ret.append(row);
   row.clear();

   // fifth row: IBU and Final gravity
   row.append( tr("IBU") );
   row.append( Brewken::displayAmount( recObs->IBU(), nullptr, 1 ) );
   row.append( tr("Final Gravity") );
   row.append( Brewken::displayAmount( recObs->fg(), PersistentSettings::Sections::tab_recipe, PropertyNames::Recipe::fg, &Units::sp_grav, 3 ) );
   ret.append(row);
   row.clear();

   // sixth row: ABV and estimate calories
   row.append(tr("ABV"));
   row.append( Brewken::displayAmount(recObs->ABV_pct(),nullptr,1) );
   row.append( Brewken::getVolumeUnitSystem() == SI ? tr("Estimated calories (per 33 cl)") : tr("Estimated calories (per 12 oz)"));
   row.append( Brewken::displayAmount(Brewken::getVolumeUnitSystem() == SI ? recObs->calories33cl() : recObs->calories12oz(),nullptr,0) );
   ret.append(row);
   row.clear();
